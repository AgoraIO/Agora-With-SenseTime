...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_createInstance(JNIEnv * env, jobject obj, jstring modelpath);
    ...
    JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect2(JNIEnv * env, jobject obj,
                                                                                      jbyteArray pInputImage, jint imageFormat, jint imageWidth, jint imageHeight, jobjectArray faceArray, jobjectArray arrAttributes);

    JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect_1ext(JNIEnv * env, jobject obj,
                                                                                         jbyteArray pInputImage, jint imageFormat, jint imageWidth, jint imageHeight, jobjectArray faceArray, jobjectArray arrAttributes);
    JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect_1ext2(JNIEnv * env, jobject obj,
                                                                                          jbyteArray pInputImage, jint imageFormat, jint imageWidth, jint imageHeight, jobjectArray faceArray, jobjectArray arrAttributes);
...
JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect2(JNIEnv * env, jobject obj,
    ...
    st_mobile_face_t *p_mobile_face = NULL;

    int len = env->GetArrayLength(faceArray);
    if (len > 0) {
        p_mobile_face = new st_mobile_face_t[len];
        for (int i = 0; i < len; ++i) {
            jobject face106 = env->GetObjectArrayElement(faceArray, i);
            if (!convert2FaceInfo(env, face106, p_mobile_face + i)) {
                memset(&p_mobile_face[i], 0, sizeof(st_mobile_106_t));
            }
            env->DeleteLocalRef(face106);
        }
    }
    LOGE("before detect： format: %d, %x, w: %d, h: %d,  stride: %d, %x, len: %d", imageFormat, srcdata, imageWidth, imageHeight, image_stride, p_mobile_face, len);
    st_mobile_attributes_t *pAttrbutes = NULL;
    int result = st_mobile_face_attribute_detect2(handle, (unsigned char *)srcdata, (st_pixel_format)imageFormat, imageWidth, imageHeight, image_stride, p_mobile_face, len, &pAttrbutes);
    env->ReleaseByteArrayElements(pInputImage, srcdata, 0);
    LOGE("result detect : %d", result);
...
JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect_1ext(JNIEnv * env, jobject obj,
    ...
    LOGE("before detect： format: %d, %x, w: %d, h: %d,  stride: %d, %x, len: %d", imageFormat, srcdata, imageWidth, imageHeight, image_stride, p_mobile_106, len);
    float *pAttrbutes = NULL;
    int result = st_mobile_face_attribute_detect_ext(handle, (unsigned char *)srcdata, (st_pixel_format)imageFormat, imageWidth, imageHeight, image_stride, p_mobile_106, len, &pAttrbutes);
    env->ReleaseByteArrayElements(pInputImage, srcdata, 0);
    LOGE("result detect : %d", result);
    ...
    if (result == ST_OK) {
        if (arrAttributes == NULL) {
            ...
            for (int i = 0; i < len; ++i) {
                jfloatArray faar = env->NewFloatArray(32);
                float* attr = &pAttrbutes[i];
                env->SetFloatArrayRegion(faar, 0, 32, attr);
                env->SetObjectArrayElement(arrAttributes, i, faar);
                env->DeleteLocalRef(faar);
            }
        }
    }
...
JNIEXPORT int JNICALL Java_com_sensetime_stmobile_STMobileFaceAttributeNative_detect_1ext2(JNIEnv * env, jobject obj,
    ...
    LOGE("before detect： format: %d, %x, w: %d, h: %d,  stride: %d, %x, len: %d", imageFormat, srcdata, imageWidth, imageHeight, image_stride, p_mobile_face, len);
    float *pAttrbutes = NULL;
    int result = st_mobile_face_attribute_detect_ext2(handle, (unsigned char *)srcdata, (st_pixel_format)imageFormat, imageWidth, imageHeight, image_stride, p_mobile_face, len, &pAttrbutes);
    env->ReleaseByteArrayElements(pInputImage, srcdata, 0);
    LOGE("result detect : %d", result);
