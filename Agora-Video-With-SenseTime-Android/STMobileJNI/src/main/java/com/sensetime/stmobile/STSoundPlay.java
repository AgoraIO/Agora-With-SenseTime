...
public class STSoundPlay {
     ...
     * @return
     */
    private STSoundPlay(Context appContext) {
    ...
    public void setEffectHandle(STMobileEffectNative effectHandle) {
        if (null == effectHandleRef) {
            effectHandleRef = new WeakReference<STMobileEffectNative>(effectHandle);
        }
    }
    ...
    public void setSoundPlayDone(String name) {
        if (effectHandleRef.get() != null) {
            effectHandleRef.get().setSoundPlayDone(name);
        }
    }
