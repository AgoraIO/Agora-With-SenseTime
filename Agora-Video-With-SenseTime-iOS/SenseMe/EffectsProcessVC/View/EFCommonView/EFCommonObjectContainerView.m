...
@interface EFCommonObjectContainerView () <EFCommonObjectViewDelegate>

@property (nonatomic, readwrite, assign) int newObjectViewID;

@end

@implementation EFCommonObjectContainerView
    ...
    if (self.faceArray.count > 0) {
        ...
        for (NSDictionary *dic in self.faceArray) {
            ...
            if ([dic objectForKey:POINT_KEY]) {
                ...
                CGContextFillRect(context, CGRectMake(point.x * self.frame.size.width - 1, point.y * self.frame.size.height - 1, 2.0, 2.0));
            }
        }
    }
...
- (void)addCommonObjectViewWithImage:(UIImage *)image {
    _currentCommonObjectView = [[EFCommonObjectView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH / 2 - COMMON_OBJCET_VIEW_WIDTH / 2, SCREEN_HEIGHT / 2 - COMMON_OBJECT_VIEW_HEIGHT / 2, COMMON_OBJCET_VIEW_WIDTH, COMMON_OBJECT_VIEW_HEIGHT) commonObjectViewID:_newObjectViewID image:image];
