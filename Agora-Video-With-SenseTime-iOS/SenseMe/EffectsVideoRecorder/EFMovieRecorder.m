...
//                @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Not recording" userInfo:nil];
			...
			case EFMovieRecorderStatusFailed:
				...
				DLog(@"Recording has failed, nothing to do");
				break;
	...
	dispatch_async(_writingQueue, ^{
		...
		@autoreleasepool {
			...
			if (input.readyForMoreMediaData) {
				...
				DLog(@"%@ input not ready for more media data, dropping buffer", mediaType);
			}
			CFRelease(sampleBuffer);
		}
	});
...
#if DEBUG
	DLog(@"EFMovieRecorder state transition: %@->%@", [self stringForStatus:_status], [self stringForStatus:newStatus]);
#endif
...
#if DEBUG
			if (error) {
				DLog(@"EFMovieRecorder error: %@, code: %i", error, (int)error.code);
			}
#endif
		}
...
- (BOOL)setupAssetWriterAudioInputWithSourceFormatDescription:(CMFormatDescriptionRef)audioFormatDescription settings:(NSDictionary *)audioSettings error:(NSError **)errorOut {
	if (!audioSettings) {
		DLog(@"No audio settings provided, using default settings");
...
- (BOOL)setupAssetWriterVideoInputWithSourceFormatDescription:(CMFormatDescriptionRef)videoFormatDescription transform:(CGAffineTransform)transform settings:(NSDictionary *)videoSettings error:(NSError **)errorOut {
	if (!videoSettings) {
		...
		DLog(@"No video settings provided, using default settings");
