...
- (void)getPhotoListWithAlbumModel:(HXAlbumModel *)albumModel
    ...
    if (self.configuration.reverseDate) {
        [result enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
            if (weakself.type == HXPhotoManagerSelectedTypePhoto && asset.mediaType != PHAssetMediaTypeImage) {
                return;
            }else if (weakself.type == HXPhotoManagerSelectedTypeVideo && asset.mediaType != PHAssetMediaTypeVideo) {
                return;
            }
            BOOL filter = NO;
            if (weakself.assetFilter) {
                filter = weakself.assetFilter(albumModel, asset);
            }
            if (!filter) {
