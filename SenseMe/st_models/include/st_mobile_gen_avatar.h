...
// API specific error.
#define ST_E_NO_AVATAR_FOUND				-42  ///< 指定avatar不存在
...
typedef enum
{
    AVATAR_FEATURE_IDX_EYE = 0,
    AVATAR_FEATURE_IDX_NOSE = 1,
    AVATAR_FEATURE_IDX_MOUTH = 2,
    AVATAR_FEATURE_IDX_FACE = 3,
...
/// @brief 导出当前avatar捏脸配置（骨骼参数，属性信息等）到文件
/// @param[in] handle 已初始化的avatar handle
...
/// @param[in] format 导出配置文件的格式，目前支持导出json、zip两种文件格式
/// @param[out] p_buffer 待写入捏脸数据的内存buffer
...
st_mobile_gen_avatar_export_pinch_config_to_buffer(
    ...
    const char *config_name,
    st_avatar_pinch_data_format format,
    char *p_buffer, int *buffer_len, bool exportInRightHand
);
...
/// @brief 通过文件路径设置avatar背景
/// @param[in] handle 已初始化的avatar handle
/// @param[in] background_path 用于背景的文件路径，若图像路径为NULL，则显示相机原图（输入的texture）
/// @param[out] package_id 返回的输入素材ID
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
ST_SDK_API st_result_t
...
/// @brief 设置avatar背景颜色
/// @param[in] handle 已初始化的avatar handle
/// @param[in] p_color 要设置的背景颜色，若该参数为NULL，则显示相机原图
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
ST_SDK_API st_result_t
