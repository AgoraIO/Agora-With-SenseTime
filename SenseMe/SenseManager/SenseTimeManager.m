...
#pragma mark SenseBeautify
    {
        ...
        self.senseBeautifyManager = senseBeautifyManager;
    }

#pragma mark SenseEffects
    {
        SenseEffectsManager *senseEffectsManager = [SenseEffectsManager new];
        self.senseEffectsManager = senseEffectsManager;
    }
}
...
//    self.licenseData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"SENSEME" ofType:@"lic"]];
    if ([self checkLicenseFromServer]) {
        ///ST_MOBILE：初始化相关的句柄
        [self setupHandle];
        [self initSenseModule];
...
- (void)setupSenseArService {
    [[SenseArMaterialService sharedInstance] authorizeWithAppID:@"6dc0af51b69247d0af4b0a676e11b5ee" appKey:@"e4156e4d61b040d2bcbf896c798d06e3" onSuccess:^{
        [self initResource];
...
- (void)setupHandle {
    ...
    iRet = st_mobile_human_action_create(NULL, config, &_hDetector);
    ...
    if (ST_OK != iRet || !_hDetector) {
        ...
        //使用新的api来实现get_detect_config
        addSubModel(_hDetector, @"M_SenseME_Face_Video_7.0.0");
        addSubModel(_hDetector, @"M_SenseME_Face_Extra_Advanced_6.0.11");
        addSubModel(_hDetector, @"M_SenseME_Iris_2.0.0");
        addSubModel(_hDetector, @"M_SenseME_Hand_5.4.0");
        addSubModel(_hDetector, @"M_SenseME_Segment_4.12.8");
        addSubModel(_hDetector, @"M_SenseME_Avatar_Help_2.2.0");
        addSubModel(_hDetector, @"M_SenseME_Segment_Hair_1.3.4");
        addSubModel(_hDetector, @"M_SenseME_MouthOcclusion_1.1.2");
        addSubModel(_hDetector, @"M_SenseME_Segment_Head_1.0.3");
#if TEST_BODY_BEAUTY
...
//    CVPixelBufferRef rgbPixelBuffer = [SenseTimeManager RGBAPixelBufferWithNV12:pixelBuffer];
...
//
//        size_t image_data_size = sizeof(unsigned char) * (*src).p_hair * (*src).p_hair.height;
